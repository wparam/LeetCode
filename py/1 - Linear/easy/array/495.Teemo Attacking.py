...
# If Teemo attacks again before the poison effect ends, the timer for it is reset, and the poison effect will end duration seconds after 
# the new attack.
...
def findPoisonedDuration(timeSeries: List[int], duration: int) -> int:
    cur = 0 # current poisoned time
    last = 0 # current poisoned sec
    l = len(timeSeries)
    for i in range(0, l):
      if timeSeries[i]<last:
        cur = cur + (duration - (last - timeSeries[i]))
        last = last + (duration - (last - timeSeries[i]))
      else:
        cur = cur + duration
        last = duration + timeSeries[i]
    # print(f"cur is {cur}, last is {last}")
