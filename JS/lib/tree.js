...
class Tree{
    ...
    serializeTree(arr){
        ...
        while(count <= arr.length-1){
            ...
            for(let j=0; j<lvs.length; j++){
                lvs[j].left = arr[count]===null ? null : new TreeNode(arr[count]);
                count++;
                if(count <= arr.length-1){
                    lvs[j].right = arr[count]===null ? null : new TreeNode(arr[count]);
                    count++;
                }
            }
        } 
    ...
    deserializeTree(tree){
        ...
        for(let k = 0; k<=lastnotnull; k++){
            numresult.push(result[k]===null ? null : result[k].val);
        }
        return numresult;
